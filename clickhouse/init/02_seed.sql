...
--
-- Dates are anchored to now()/today() AT LOAD TIME and spread backwards over a
-- rolling window, so relative-date questions ("yesterday", "last 7 days") always
-- return data. Recreate the container (docker compose down -v && up -d) to refresh
-- the dates to the current day.

INSERT INTO analytics.users
SELECT
    number + 1                                            AS user_id,
    today() - toIntervalDay(number % 150)                 AS signup_date,
...
SELECT
    ...
    now() - toIntervalSecond(intDiv(number * (90 * 86400), 2000))       AS started_at,
